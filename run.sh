...
# Применение миграций Django удалено, так как база данных не используется явно.
# Django по умолчанию использует SQLite, который не требует предварительных миграций для запуска.
